...
| MLLP close budget | 8s | Listener: time budget on `SIGTERM`/`SIGINT` to wait for already-accepted MLLP connections to close before forcing the still-open ones closed (`close_clients()`), which is what bounds shutdown. |
